...
class MaskedMultiAttention(nn.Module):
    ...
    def forward(self, output, enc_output, enc_input=[], flag=1):
        ...
        tri_pad_weight = torch.ones(self.batch, dec_T, T, dtype=torch.uint8, device=device)
        tri_pad_weight = torch.triu(tri_pad_weight, diagonal=1)
        pad_weight = (pad_weight | tri_pad_weight)
